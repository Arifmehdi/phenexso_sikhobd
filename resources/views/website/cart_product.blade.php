...
<div class="cart-page">
    <div class="container">
        ...
        <form action="{{ route('productOrderStore') }}" method="POST" id="checkoutForm">
            @csrf
                    ...
                    <div class="modern-card">
                        ...
                        <div class="form-content">
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    <label class="custom-label">{{ app()->getLocale() == 'bn' ? 'বিস্তারিত ঠিকানা (বাসা, রোড, এলাকা ও জেলা) *' : 'Full Address (House, Road, Area & District) *' }}</label>
                                    <textarea name="billing_address" class="form-control custom-input" rows="4" placeholder="{{ app()->getLocale() == 'bn' ? 'আপনার বিস্তারিত ঠিকানা লিখুন' : 'Enter your full address' }}" required>{{ auth()->check() && auth()->user()->locations()->first() ? auth()->user()->locations()->first()->address_title : '' }}</textarea>
                                </div>

                                <div class="col-12">
                                    <label class="custom-label">{{ app()->getLocale() == 'bn' ? 'অফিস ঠিকানা (ঐচ্ছিক)' : 'Office Address (Optional)' }}</label>
                                    <textarea name="office_address" class="form-control custom-input" rows="2" placeholder="{{ app()->getLocale() == 'bn' ? 'অফিস ঠিকানা লিখুন' : 'Enter office address' }}"></textarea>
                                </div>

                                <div class="col-12">
                                    <label class="custom-label">{{ app()->getLocale() == 'bn' ? 'অফিস সময় (ঐচ্ছিক)' : 'Office Time (Optional)' }}</label>
                                    <input type="text" name="office_time" class="form-control custom-input" placeholder="{{ app()->getLocale() == 'bn' ? 'যেমন: সকাল ১০টা - রাত ৮টা' : 'e.g. 10 AM - 8 PM' }}">
                                </div>

                                <div class="col-12">
                                    <label class="custom-label">{{ app()->getLocale() == 'bn' ? 'অর্ডার নোট (ঐচ্ছিক)' : 'Order Note (Optional)' }}</label>
                                    <textarea name="order_note" class="form-control custom-input" rows="2" placeholder="{{ app()->getLocale() == 'bn' ? 'অর্ডার সম্পর্কে বিশেষ কোনো তথ্য থাকলে দিন' : 'Enter any special instructions' }}"></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        const codRoute = "{{ route('productOrderStore') }}";
