                ...
                <td>
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                            <a href="{{ $category->type == 'course' ? route('admin.courseCategories.edit', $category) : route('admin.productCategories.edit', $category) }}" class="dropdown-item"><i class="fa fa-edit"></i> Edit</a>
                            <form action="{{ $category->type == 'course' ? route('admin.courseCategories.destroy', $category) : route('admin.productCategories.destroy', $category) }}" method="post" onsubmit="return confirm('Delete this category and all sub-items?')">
                                @csrf
                                @method('DELETE')
                ...
                <tr class="collapse child-of-{{ $category->id }} bg-light" data-id="{{ $child->id }}" data-toggle="collapse" data-target=".grandchild-of-{{ $child->id }}">
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a href="{{ $child->type == 'course' ? route('admin.courseCategories.edit', $child) : route('admin.productCategories.edit', $child) }}" class="dropdown-item"><i class="fa fa-edit"></i> Edit</a>
                                <form action="{{ $child->type == 'course' ? route('admin.courseCategories.destroy', $child) : route('admin.productCategories.destroy', $child) }}" method="post" onsubmit="return confirm('Are you sure?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="dropdown-item text-danger"><i class="fa fa-trash"></i> Delete</button>
                                </form>
                    ...
                    <tr class="collapse grandchild-of-{{ $child->id }} bg-white">
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a href="{{ $grandchild->type == 'course' ? route('admin.courseCategories.edit', $grandchild) : route('admin.productCategories.edit', $grandchild) }}" class="dropdown-item px-2"><i class="fa fa-edit text-info"></i> Edit</a>
                                    <form action="{{ $grandchild->type == 'course' ? route('admin.courseCategories.destroy', $grandchild) : route('admin.productCategories.destroy', $grandchild) }}" method="post" onsubmit="return confirm('Are you sure?')">
                                        @csrf
                                        @method('DELETE')
